...
- **Surface vegetation**: decorative **tall grass** and red/yellow **flowers** scatter across **Plains** grass as **cross-quad** ("X"-shaped) cutout plants — two crossed vertical quads with a transparent background, drawn into the chunk's solid mesh with alpha-test cutout so the gaps render cleanly. Placement is deterministic per seed (a handful of candidate cells per chunk, gated on a grass surface above sea level), roughly five tufts of grass for every flower. They're **non-solid** (you walk straight through them) but **breakable**, and each drops itself when broken
