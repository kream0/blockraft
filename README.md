...
- **Sky lighting** (Lighting v1): a per-block sky-light level (0–15) floods straight down open columns, then a 6-neighbour BFS spreads it around corners (attenuating one step per block), so caves, overhangs, and deep interiors sink into shadow while exposed surfaces stay fully lit. The level is baked into per-vertex brightness (multiplying the AO term) and recomputed per chunk whenever it meshes, pulling light across borders from loaded neighbours so seams stay smooth; the day/night ambient still scales on top. **Block light** (Lighting v2) now floods the same way from light-emitting blocks: a placed **torch** emits level-14 light through an independent BFS, kept in a separate per-chunk grid and combined with sky light as `max(sky, block)` only when a chunk meshes — so torches brighten caves, overhangs, and night-time interiors that sky light never reaches. (Emitted light is neutral/grayscale for now; warm/colored light is a later pass)
...
- 20 block types: Grass, Dirt, Stone, Cobblestone, Wood, Leaves, Planks, Sand, Snow, Glass, Bedrock, Water, Coal Ore, Iron Ore, Diamond Ore, Furnace, Chest, Door, Torch, Air
...
- **Lighting (more emitters / colored light)**: additional light-emitting blocks (glowstone, lava) and warm/colored emission — **block light** (Lighting v2: a craftable **torch** that floods level-14 light through the same BFS, combined with sky light at mesh time), **per-block sky light** (Lighting v1: a BFS sky-light flood baked into vertex brightness so caves darken), and vertex ambient occlusion / smooth contact shading all already ship (see World above)
