...
- **Visual-realism overhaul** (in progress): the **graphics-settings foundation** (quality presets + an 18-knob advanced panel, validated and persisted) and the **post-processing pipeline** now ship — an `EffectComposer` on an HDR half-float target with runtime anti-aliasing (FXAA / SMAA), ACES / linear tone mapping, bloom, exp2 fog, and tiered shadow maps + PCF softness, all switchable live from the presets (see Menus & UX above). The remaining work adds tiered atlas resolution + anisotropic filtering, normal/roughness mapping with bevelled edges, richer water + atmosphere, and SSAO. Vertex ambient occlusion, baked sky/block light, and screen-space fog already ship (see World above)
