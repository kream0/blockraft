...
- **Surface vegetation**: decorative **tall grass** and red/yellow **flowers** scatter across **Plains** grass. Tall grass renders as a **cross-quad** ("X"-shaped) cutout — two crossed vertical quads with a transparent background and alpha-test cutout so the gaps read cleanly. **Flowers render as a true 3D model** instead — a thin **stem** box rising from the cell's bottom center topped by a wider **petal head** — so a flower reads as a single upright plant rather than a billboard painted across the cube's faces. Both are drawn into the chunk's solid mesh, and the held model and inventory icon match the in-world look. Placement is deterministic per seed (a handful of candidate cells per chunk, gated on a grass surface above sea level), roughly five tufts of grass for every flower. They're **non-solid** (you walk straight through them) but **breakable**, and each drops itself when broken
...
- **Desert temples**: rare **stepped sandstone pyramids** that generate on flat, dry **Desert** sand above sea level — a solid 9×9 base tapering layer-by-layer to a single-block apex, raised on a buried sandstone foundation so it sits level on uneven dunes. Sealed underneath is a **5×5 sandstone treasure chamber** holding a single **loot chest** (same deterministic loot system as dungeons), with no surface entrance — you mine down to it. Confined to the chunk interior so a temple never spans a border (deterministic per seed; one possible per chunk, gated on biome, dryness, flatness, and a sand surface)
...
- 28 block types: Grass, Dirt, Stone, Cobblestone, Wood, Leaves, Planks, Sand, Sandstone, Snow, Glass, Bedrock, Water, Lava, Coal Ore, Iron Ore, Diamond Ore, Furnace, Chest, Door, Torch, Glowstone, Bed, Cactus, Tall Grass, Red Flower, Yellow Flower, Air
- Procedurally generated texture atlas (16×16 base tiles, upscalable to 32² / 64² with anti-bleed gutters via graphics settings; no external image assets)
...
- **Torches attach to the face you target**: aim at a block's side and the torch mounts to that wall **canted outward** (leaning away from the wall, like Minecraft wall torches) instead of sitting dead-center in the cell; aim at a top face and it stands **upright on the floor**; ceilings are rejected (no upside-down torches). However it's mounted, breaking it always drops the normal torch item
...
- **Sprint-attack knockback**: hit a mob while **sprinting** and it's shoved back noticeably harder — a spacing tool to knock enemies off ledges or buy room. (Like sprinting itself, this needs hunger above 6 in Survival, so a starving player who can only walk won't get the extra shove)
...
- **Item-name tooltips**: hovering any occupied slot — in the inventory, the 3×3 crafting grid, a chest, or a furnace — pops a small label with the item's name (block names are prettified, e.g. `COAL_ORE` → "Coal Ore"), so you can identify a stack without recognizing its icon
- **Inventory & crafting screen** (both modes): press **E** to open a grid of all 36 slots alongside a 3×3 crafting grid; rearrange with a held cursor stack — left-click picks up / drops / merges / swaps, right-click splits a stack in half or drops one, and **left-click-drag across several slots evenly distributes the held stack** among them (the remainder stays on the cursor); close with **E** or **Esc** (gameplay soft-pauses while it's open, and leaving with ESC re-grabs the mouse on your **next key press or click** — same as resuming from the pause menu with ESC — so you can look around immediately without clicking the canvas)
...
- **Directional sun shadows**: the sun is a real shadow-casting light, so terrain throws **dynamic cast shadows** that swing with the time of day — long and raking at dawn/dusk, short at noon. Because the terrain material is **unlit** (brightness comes entirely from baked voxel light), Three's shadow machinery is hand-injected into the chunk shader via `onBeforeCompile`: the shadow factor multiplies **only the sky-lit channel** (never torch/block light) and is scaled by the live day/night uniform, so shadows fade out at night and torch-lit interiors are never darkened. An orthographic shadow frustum re-centers on the player every frame and reaches deep enough that low terrain still receives; a soft ambient floor keeps shadowed faces readable rather than pure black
...
- **Settings** (live-applied during gameplay):
  ...
  - **Graphics quality**: a one-click preset selector (**Low / Medium / High / Ultra**) backed by an expandable **Advanced** panel of 21 knobs — anti-aliasing, shadow-map size + softness, SSAO (+ samples + intensity), normal maps, edge rounding, atlas tile size, anisotropic filtering, tone mapping, fog type, bloom (+ intensity + threshold), emissive block glow, pixel-ratio cap, water quality, cloud detail, and god rays (+ strength). Picking a preset fills every knob at once; tweaking any single knob flips the selector to **Custom** (and auto-opens the panel). The controls, validation, and per-setting persistence shipped first as the foundation for the **visual-realism overhaul**; each knob then wires into the renderer progressively. **Live now** — driving a real post-processing pipeline (`EffectComposer` on an HDR target, switchable mid-game): anti-aliasing (FXAA / SMAA), tone mapping (none / linear / **ACES**), bloom (+ intensity + threshold), fog type (linear / exp2), shadow-map size tiers, shadow softness (PCF / PCF-soft), and the pixel-ratio cap — plus **tiered texture resolution**: atlas tile size **16 / 32 / 64 px** with anti-bleed UV gutters, and **anisotropic filtering** up to the GPU max. Changing the tile size rebuilds the atlas and live-remeshes every loaded chunk behind a HUD progress banner; anisotropy applies instantly. Terrain now renders through a physically-based **`MeshStandardMaterial`** that keeps the baked, leak-free voxel lighting (and day/night shadows) authoritative with terrain rendered **fully matte** for a clean voxel look — no sun specular, which removed a sun-synced highlight band that swept across surfaces and a grazing-angle "wet" sheen on distant vertical faces. (The **procedural normal + roughness maps** and **edge rounding** — analytic bevel at Medium, tangent-space normal-mapped bevel at High/Ultra — still build but, with terrain matte, now shape only the **water** surface.) **Water** now renders through the same physically-based shader with three quality tiers — **basic** (still, plain translucency), **animated** (a time-scrolled procedural surface ripple, a Fresnel rim that turns water more opaque at grazing angles, and a moving sun glint), and **reflective** (adds a day/night-scaled sky tint on top) — switchable live, with only the reflective boundary needing a shader recompile. The sky is now a **gradient dome** — an always-on, camera-following skybox whose horizon tracks the live sky colour (seamless with the fog) while the zenith darkens for atmospheric depth, with a warm sun-direction halo that fades out at night and blooms through the post pipeline for free. **Cloud detail** (low / medium / high / ultra) now drives the procedural cloud sheet's resolution and blob density — higher tiers paint a denser, crisper layer, regenerated in place with no remesh when you change it. **Emissive block glow** (on at High/Ultra) makes torches, glowstone and lava push their block-lit faces past 1.0 in HDR so the bloom pass makes them visibly glow at night — a gated, live-toggled shader term that reuses the existing baked block-light channel (no new attribute, no remesh, and sky-lit daytime faces never glow). **God rays** (on at High/Ultra) add screen-space crepuscular sun shafts — a single radial light-scatter post pass, slotted after the scene and before bloom, that fans light around terrain silhouettes from the sun's projected screen position and fades in/out smoothly as the sun drops below the horizon or pans off-frame (strength is a live slider; the pass is an exact passthrough — near-zero cost — whenever the sun is off-screen). **Scaffolded but inert** pending later phases: SSAO. The pipeline details live in `src/rendering/GRAPHICS.md`
...
- **Visual-realism overhaul** (in progress): the **graphics-settings foundation** (quality presets + an 18-knob advanced panel, validated and persisted), the **post-processing pipeline** (an `EffectComposer` on an HDR half-float target with runtime anti-aliasing (FXAA / SMAA), ACES / linear tone mapping, bloom, exp2 fog, and tiered shadow maps + PCF softness), **tiered atlas resolution + anisotropic filtering**, and a physically-based **`MeshStandardMaterial`** terrain shader with **procedural normal/roughness maps and bevelled edges** (preserving the baked voxel lighting + day/night shadows), and richer **water** (a scrolling ripple normal, a Fresnel opacity rim, a moving sun glint, and an optional sky-tint reflection at Ultra), and a first slice of **atmosphere** — a **gradient sky dome** (a camera-following skybox with a darkened zenith, a fog-matched horizon, and a warm sun-direction halo) plus **cloud-detail tiers** (cloud quality now drives the procedural cloud sheet's resolution + blob density), and **emissive block glow** (block-lit torch / glowstone / lava faces bloom past 1.0 in HDR so they glow at night, on at High/Ultra), and **god rays** (screen-space crepuscular sun shafts from a radial light-scatter post pass that fans around terrain silhouettes, on at High/Ultra) — now ship — all switchable live from the presets (see Menus & UX above). The remaining work adds **SSAO**. Vertex ambient occlusion, baked sky/block light, and screen-space fog already ship (see World above)
...
| **Right click (hold)** | Place block (hold to keep placing), or open a door / chest / furnace |
