...
- **Visual-realism overhaul** (in progress): the **graphics-settings foundation** (quality presets + an 18-knob advanced panel, validated and persisted), the **post-processing pipeline** (an `EffectComposer` on an HDR half-float target with runtime anti-aliasing (FXAA / SMAA), ACES / linear tone mapping, bloom, exp2 fog, and tiered shadow maps + PCF softness), **tiered atlas resolution + anisotropic filtering**, and a physically-based **`MeshStandardMaterial`** terrain shader with **procedural normal/roughness maps and bevelled edges** (preserving the baked voxel lighting + day/night shadows) now ship — all switchable live from the presets (see Menus & UX above). The remaining work adds richer **water** (scrolling normals, Fresnel, reflections) + **atmosphere**, **emissive blocks**, and **SSAO**. Vertex ambient occlusion, baked sky/block light, and screen-space fog already ship (see World above)
