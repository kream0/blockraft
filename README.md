...
- **Lighting (more emitters / colored light)**: additional light-emitting blocks (glowstone, lava) and per-emitter *colored* light are the remaining work — **block light** (Lighting v2: a craftable **torch** that floods level-14 light through the same BFS, combined with sky light at mesh time, now rendered as a **warm firelight-orange emissive**), **smooth per-vertex light diffusion** (each vertex averages its neighbour light cells so light fades off in a gradient rather than tinting whole faces), **per-block sky light** (Lighting v1: a BFS sky-light flood baked into vertex brightness so caves darken), and vertex ambient occlusion / smooth contact shading all already ship (see World above)
